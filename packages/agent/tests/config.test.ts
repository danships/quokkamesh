...
describe('config', () => {
  it('loadConfig with no file returns empty object', () => {
    const config = loadConfig('/nonexistent/path/qmesh.config.json');
    expect(config).toEqual({});
  });

  it('resolveDataDir uses config.dataDir when set', () => {
    const orig = process.env['QMESH_DATA_DIR'];
    try {
      delete process.env['QMESH_DATA_DIR'];
      const dir = resolveDataDir({ dataDir: '/custom/path' });
      expect(dir).toContain('custom');
      expect(dir).toContain('path');
    } finally {
      if (orig !== undefined) {
        process.env['QMESH_DATA_DIR'] = orig;
      }
    }
  });
